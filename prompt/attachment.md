...
#### Bug 修复：连续录音 ASR 超时
**问题描述**：第一次录音成功，紧接着第二次录音就 ASR 超时失败，间隔久一点可以成功

**根因分析**：
- FunASR WebSocket 连接在第一次转录后未断开
- 第二次转录时复用旧连接，但 FunASR 服务器期望新连接开始新会话
- 旧连接不响应，导致 30 秒后超时

**修复方案**：
- [x] 在 `resetState()` 中添加 `disconnect()` 断开 WebSocket 连接
- [x] 确保每次 ASR 转录完成后断开连接，下次使用全新连接

**修复日期**：2024-02-12
**测试结果**：✅ 可以连续快速录音，每次都能成功转写

#### 第二阶段：相机拍摄上传（方案A）
- [x] UI 改造：录音按钮左侧添加相机图标按钮 ✓
- [x] 相机权限和调用：系统相机集成 ✓  
- [x] 照片处理：拍摄、压缩、Base64 编码 ✓
- [x] 防误触保护：防止在录音时触发相机操作 ✓
- [x] 临时文件管理：拍照后自动清理缓存文件 ✓

**实施日期**：2025-02-12
**技术实现**：
1. **权限与配置**：
   - 添加 CAMERA 权限到 AndroidManifest.xml
   - 配置 FileProvider 用于安全文件共享
   - 添加相机硬件特性声明（optional）
   
2. **相机功能**：
   - 相机按钮点击事件绑定
   - 动态相机权限请求和处理
   - 系统相机调用（ACTION_IMAGE_CAPTURE）
   - 临时照片文件创建和管理
   
3. **照片处理流程**：
   - 拍照结果通过 `onActivityResult` 接收
   - 复用现有文件上传管道（`processSelectedFiles`）
   - Base64 编码和 WebView 注入
   - 临时文件自动清理
